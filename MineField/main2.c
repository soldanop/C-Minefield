...
int main(){
intro();
int pilot; /*Variabile che permette di tornare indietro nel menu quando si perde o quando si vince*/
char player[10]; /*Vettore per contenere il nome del player*/
pilot= menu_principale(-1,player); /*Variabile che contiene la scelta che l'utente fa nel menu*/
while(pilot!=5){
int a=pilot; /*Variabile che salva la scelta dell'utente nella funzione menu principale*/
pilot=0; 
FILE* f; /*Variabile per il file */
char **vett=NULL;	/*Campo della matrice visibile al player*/
char **mine=NULL;	/*Campo della matrice dove sono situati le bombe, gli spazi e i numeri*/
char nomefile[10]; /*Vettore per contenere il nome del file*/
int **back_track=NULL; /*Campo per la funzionalità del back_track*/
...
size_t byte=1; int temp=-1; int temp1=-1; /*Variabili utilizzate esclusivamente per tutte le getline*/ 
...
if(a==1){   /*Se scelgo di CARICARE una PARTITA*/
	system("clear");
	...
	if(f==NULL){ /*Controllo se il file è stato aperto corretamente*/
...
if(a==2){ /*Se scelgo di GENERARE un campo*/
	system("clear");
...
if(a==2){ /*SCelta di generare un campo*/
	pulisci_campo(vett, colonne, righe); /*Prepara il campo mettendo in tutte le celle dei punti interrogativi*/
	pulisci_campomine(mine, colonne, righe); /*Prepara il campo delle mine mettendo in tutte le celle degli spazi vuoti*/
...
if (f!=NULL && a==1){	/*Preparativi per la scelta 1 (CARICA CAMPO) */
...
if(a==1|| a==2){
	while(exit!=0 && pilot==0){ /*while della partita -> esce quando il giocatore ha vinto o quando il giocatore vuole tornare nel menu*/
		exit=0;
		do{
			...
			if (scelta==3){
				printf(specialyellow"\n\nEnd\n"reset);
				free(vett); free(mine); free(back_track);
				return 0;
			}
	
			printf(specialred"Inserisci le coordinate\n"); /*Coordinate che inserisce l'utente*/
			x=righe+1;
			y=colonne+1;
			free(zol);
			zol=NULL;
			...
			if(back_track[x][y]!=0){	/*Nel caso l'utente tenta di scoprire una cella già scoperta*/
				system("clear");
				stampa(vett,righe,colonne,back_track,mine);
		...
		if(scelta==1 && pilot==0){	/*Caso se l'utente ha deciso di scoprire una cella*/
			nuovoturno(back_track,righe,colonne);
			if(mine[x][y]=='*'&& vett[x][y]=='?'){	/*Caso se l'utente ha preso una bomba*/
            	system("reset");
                stampa_bombe(mine,colonne,righe);
				printf(specialred"\nGAME OVER "reset);
				if(aiuti>0){
					printf(specialyellow"1 per tornare indietro\t2 per uscire dalla partita\n"reset);
					scanf("%d", &i);
					if (i==1){
						...
						fclose(f);
						 a=-1;
						pilot=menu_principale(-1,player);
			...
			if(mine[x][y]==' '&& vett[x][y]=='?' && pilot==0) /*Se l'utente scopre una cella dove cè uno spazio*/
				cerca_spazi(x,y,mine,vett,righe,colonne,back_track);	
		...
		if(scelta==2 && pilot==0){	/*Inserisco la bandiera nella coordinata che ha deciso l'utente*/
			if (vett[x][y]=='?'&& sommaflag>0){
				vett[x][y]='/';
				check=1;
				sommaflag--;
			}
			if(vett[x][y]=='/' && check!=1){	/*Rimuovo la bandiera nella coordinata che ha deciso l'utente*/
				vett[x][y]='?';
				sommaflag++;
			}
		}
		
		for (i = 0; i < righe; ++i){			/*Ciclo che controlla se ho vinto*/
			for (j = 0; j < colonne; ++j){
				if (vett[i][j]=='?'){
					exit=1;
				}
		...
		if(exit==0 && pilot==0){	/*Caso quando l'utente vince*/
			printf(specialred"Hai vinto\t \t \t "reset);
...
if(a==3){	/*Scelta del Record*/
char testo[100];	/*Vettore che salva i nomi dei players*/
char testo1[100]; /*Vettore che salva il risultato della partita */int b;/*Salvo la scelta di resettare i record o meno*/
   ...
   if (f!=NULL){
		...
		do{
   		...
   		printf(specialgreen"ERRORE NEL CARICAMENTO DEL FILE\n"reset); /*Caso se il file non è statpo aperto correttamente*/

	pilot=menu_principale(-1,player);
}

if(a==4){ /*Scelta 4  Record*/
	int credits=-1;
	credit();
	printf(specialblue"\nMenu (1) EXIT (2)\n"reset);
	do{
